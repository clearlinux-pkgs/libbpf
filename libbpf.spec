#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : libbpf
Version  : 0.3
Release  : 5
URL      : https://github.com/libbpf/libbpf/archive/v0.3/libbpf-0.3.tar.gz
Source0  : https://github.com/libbpf/libbpf/archive/v0.3/libbpf-0.3.tar.gz
Summary  : No detailed summary available
Group    : Development/Tools
License  : BSD-2-Clause GPL-2.0 LGPL-2.1
Requires: libbpf-lib = %{version}-%{release}
Requires: libbpf-license = %{version}-%{release}
BuildRequires : elfutils-dev
BuildRequires : pkgconfig(libelf)
BuildRequires : pkgconfig(zlib)
Patch1: 0001-Fix-build-with-latest-gcc-binutils-with-LTO.patch

%description
BPF/libbpf usage and questions
==============================
Please check out [libbpf-bootstrap](https://github.com/libbpf/libbpf-bootstrap)
and [the companion blog post](https://nakryiko.com/posts/libbpf-bootstrap/) for
the examples of building BPF applications with libbpf.
[libbpf-tools](https://github.com/iovisor/bcc/tree/master/libbpf-tools) are also
a good source of the real-world libbpf-based tracing tools.

%package dev
Summary: dev components for the libbpf package.
Group: Development
Requires: libbpf-lib = %{version}-%{release}
Provides: libbpf-devel = %{version}-%{release}
Requires: libbpf = %{version}-%{release}

%description dev
dev components for the libbpf package.


%package lib
Summary: lib components for the libbpf package.
Group: Libraries
Requires: libbpf-license = %{version}-%{release}

%description lib
lib components for the libbpf package.


%package license
Summary: license components for the libbpf package.
Group: Default

%description license
license components for the libbpf package.


%prep
%setup -q -n libbpf-0.3
cd %{_builddir}/libbpf-0.3
%patch1 -p1

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1609742065
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FCFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=4 "
export FFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=4 "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=4 "
pushd src
make  %{?_smp_mflags}
popd


%install
export SOURCE_DATE_EPOCH=1609742065
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/package-licenses/libbpf
cp %{_builddir}/libbpf-0.3/LICENSE.BSD-2-Clause %{buildroot}/usr/share/package-licenses/libbpf/34c5034377edef1080538bd0d4f5cf9b78e22dff
cp %{_builddir}/libbpf-0.3/LICENSE.LPGL-2.1 %{buildroot}/usr/share/package-licenses/libbpf/91c66db733cf0ff2b3216ec4223b940daf6b26d4
pushd src
%make_install
popd

%files
%defattr(-,root,root,-)

%files dev
%defattr(-,root,root,-)
/usr/include/bpf/bpf.h
/usr/include/bpf/bpf_core_read.h
/usr/include/bpf/bpf_endian.h
/usr/include/bpf/bpf_helper_defs.h
/usr/include/bpf/bpf_helpers.h
/usr/include/bpf/bpf_tracing.h
/usr/include/bpf/btf.h
/usr/include/bpf/libbpf.h
/usr/include/bpf/libbpf_common.h
/usr/include/bpf/libbpf_util.h
/usr/include/bpf/xsk.h
/usr/lib64/libbpf.so
/usr/lib64/pkgconfig/libbpf.pc

%files lib
%defattr(-,root,root,-)
/usr/lib64/libbpf.so.0
/usr/lib64/libbpf.so.0.3.0

%files license
%defattr(0644,root,root,0755)
/usr/share/package-licenses/libbpf/34c5034377edef1080538bd0d4f5cf9b78e22dff
/usr/share/package-licenses/libbpf/91c66db733cf0ff2b3216ec4223b940daf6b26d4
